namespace LootOverhaul.Gate
{
    ...
    public class ModPeer
    {
        ...
        /// <summary>Null when this player advertises no <c>lo.*</c> properties — i.e. not running LootOverhaul.</summary>
        ...
        public override string ToString() =>
            IsModded
                ? $"actor {ActorNumber} `{NickName}` — lo {Version} / {Sha} / caps {ModCapsInfo.Describe(Caps)}"
                : $"actor {ActorNumber} `{NickName}` — no LootOverhaul (no lo.* properties)";
    }
}
