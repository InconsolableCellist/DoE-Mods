...
{
    /// <summary>
    /// Scans <c>UserData/CustomAvatars/Avatars/</c> for exporter output and validates it.
    ///
    ...
    public class AvatarLibrary
    {
        ...
        public static string AvatarsDir => ModPaths.AvatarsDir;
        ...
        /// <summary>
        /// The avatar the player has chosen, or the first installed one if they haven't chosen.
        /// Null only when nothing is installed. Everything that needs to know what to wear asks
        /// here, so "no choice made" resolves the same way everywhere.
        /// </summary>
        public string SelectedName
        {
            get
            {
                var chosen = ModConfig.SelectedAvatar.Value;
                if (!string.IsNullOrWhiteSpace(chosen))
                {
                    var manifest = Get(chosen);
        ...
        public void Rescan()
        {
            ...
            if (_byName.Count > 1 && string.IsNullOrWhiteSpace(ModConfig.SelectedAvatar.Value))
            {
                Core.Log.Warning($"*** {_byName.Count} avatars installed but Avatar is empty, " +
                                 $"so `{First()?.name}` is being used by default. Press F2 to cycle, " +
                                 "or set Avatar in MelonPreferences.cfg. Installed:");
                foreach (var kv in _byName) Core.Log.Warning($"      {kv.Value.name}");
            }
            else if (!string.IsNullOrWhiteSpace(ModConfig.SelectedAvatar.Value))
            {
                var chosen = Get(ModConfig.SelectedAvatar.Value);
                if (chosen == null)
                    Core.Log.Warning($"*** Avatar is `{ModConfig.SelectedAvatar.Value}` " +
