...
namespace PartyHealth
{
    ...
    ///
    /// Unlike the LootOverhaul copy this one takes an exact <see cref="MethodInfo"/>, because
    /// it is the safer of the two: the pointer is that of the exact overload given, not the first
    /// one with that name. It comes from the interop-generated method-info field. Every target here
    /// is a PunRPC on AvatarPlayer with a real body, so none should be refused; the log says.
    /// </summary>
