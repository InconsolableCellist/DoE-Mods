...
{
    /// <summary>
    /// Session-scoped recon transcript under <c>UserData/Descent/recon/</c>. The file is
    /// the primary sink; the console (shared with every other mod in MelonLoader's single
    /// Latest.log, each line prefixed <c>[Descent]</c>) gets headlines only.
    /// </summary>
    public static class ReconLog
    {
        ...
        private static StreamWriter Writer
        {
            get
            {
                ...
                _writer.WriteLine($"# Descent recon — {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
