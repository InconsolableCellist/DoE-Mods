...
namespace VisualCues
{
    ...
    /// one of its targets (<c>AudioManager.PlaySoundAt</c>) has two overloads with the same
    /// parameter count. The pointer comes from the interop-generated method-info field, so it
    /// is the pointer of that overload and not of the first one with that name.
    /// </summary>
